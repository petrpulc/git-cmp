def check(args, original, new, blob_mapping, blob_info):
    # check of files
    print("\n=== Blobs")
    import difflib
    ...
    def comp_n_diff(cmp1, cmp2, diff1, diff2, note):
        if cmp1 != cmp2:
            print("    Contents of file {} in commit {} do not match!".format(blob_info[o_blob]['path'],
                                                                              blob_info[o_blob]['commit']))
            if args.v:
                print("Diff{}:".format(note))
                print(''.join(d.compare(diff1, diff2)))
            exit(1)

    for o_blob, n_blob in blob_mapping.items():
